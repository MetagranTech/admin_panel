...
export default function RevenueDashboard() {
  ...
  const chart = (Array.isArray(data.dailyRevenue) ? data.dailyRevenue : []).map((item) => ({ name: item._id, revenue: item.revenue }));
  return <div className="space-y-8">
    <div className="card"><p className="text-slate-500">Total captured revenue</p><h2 className="text-4xl font-bold">₹{Number(data.stats?.totalRevenue ?? 0).toLocaleString('en-IN')}</h2></div>
